...
public:
    ...
    void cook() {
        while (simulation_running) {
            ...
            // Пауза между приготовлениями (имитация времени готовки)
    ...
    void fatman(int id) {
        while (simulation_running) {
            ...
            // Небольшая пауза для имитации еды
            std::this_thread::sleep_for(std::chrono::milliseconds(10));
        }
    }
...
int main() {
    ...
    std::cout << "ТРИ ТОЛСТЯКА" << std::endl;
    std::cout << "========================" << std::endl;
    std::cout << "Используется стандартный std::mutex" << std::endl;
    std::cout << "========================" << std::endl;

    // Три набора коэффициентов для разных исходов
    struct Scenario {
        int gluttony;
        int efficiency;
        std::string name;
    ...
    std::cout << "Синхронизация работает корректно." << std::endl;
